
# Hi, I'm Chandu Bobbili

**Senior Software Engineer** · Hyderabad, India  
Building full-stack systems that scale — and measuring success in business outcomes, not just shipped features.

[![Portfolio](https://img.shields.io/badge/Portfolio-chandubobbili.dev-0d1117?style=for-the-badge&logo=googlechrome&logoColor=white)](https://chandubobbili.dev)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/chandu-bobbili-15863319b)
[![Resume](https://img.shields.io/badge/Resume-Download-1f6feb?style=for-the-badge&logo=googledrive&logoColor=white)](https://drive.google.com/file/d/1zhpAwC_QXOsiDV4zdq7Dbn0GF-DPsMKa/view?usp=drivesdk)
[![Email](https://img.shields.io/badge/Email-chandubobbili12@gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:chandubobbili12@gmail.com)

---

## About me

I'm an SDE who cares about **measurable impact**. At **Deepta AI**, I've helped platforms handle **1M+ events/sec**, onboard **20,000 students** across **4 universities**, and process **4,000 fee renewals** — while cutting per-applicant handling from **15 to 8 hours**.

Earlier at **Apxor**, a dashboard migration (**Lighthouse 54 → 97**) directly retained **5+ accounts** that were ready to leave.

**Currently:** Senior SDE @ Deepta AI  
**Open to:** Full-time · Remote · Relocation

---

## Impact at a glance

| Metric | Result |
|--------|--------|
| Analytics ingestion | **1M+ events/sec** (load-tested) |
| Leads generated | **50,000+** |
| Student enrollments | **20,000** across 4 universities |
| App processing time | **15h → 8h** per applicant |
| Performance | **Lighthouse 54 → 97** |
| Customer retention | **5+ accounts** saved from churn |

---

## What I work with

**Languages:** TypeScript · Go · JavaScript · Python  

**Frontend:** React · Next.js · TanStack · Tailwind CSS · Shadcn UI  

**Backend:** Microservices · REST · Kafka · gRPC · SSE · BFF  

**Data & infra:** PostgreSQL · MongoDB · ClickHouse · Redis · Docker · Kubernetes (GKE) · GCP

---

## Featured projects

### [ZenithUI](https://github.com/ChanduBobbili/ZenithUi)
Open-source React component library — TypeScript-first, tree-shakeable, WAI-ARIA compliant, Tailwind-ready theming.

[![npm](https://img.shields.io/npm/dw/@zenithui/day-picker)](https://www.npmjs.com/package/@zenithui/day-picker)
[![stars](https://img.shields.io/github/stars/ChanduBobbili/ZenithUI?style=flat)](https://github.com/ChanduBobbili/ZenithUi)
[![docs](https://img.shields.io/badge/docs-zenithui.chandubobbili.dev-34D399?style=flat)](https://zenithui.chandubobbili.dev)

### [changesetgoo](https://github.com/ChanduBobbili/changesetgoo)
Lightweight Go CLI for semantic versioning & changelog management — single binary, no Node.js runtime.

[![stars](https://img.shields.io/github/stars/ChanduBobbili/changesetgoo?style=flat)](https://github.com/ChanduBobbili/changesetgoo)
[![go report](https://goreportcard.com/badge/github.com/ChanduBobbili/changesetgoo)](https://goreportcard.com/report/github.com/ChanduBobbili/changesetgoo)

---

## Writing

- [Mastering SEO for Next.js: A Practical Optimisation Checklist](https://dev.to/chandu_bobbili_06/mastering-seo-for-nextjs-a-practical-optimization-checklist-4nld)
- [Biome Has Entered the Chat: A New Tool to Replace ESLint and Prettier](https://dev.to/chandu_bobbili_06/biome-has-entered-the-chat-a-new-tool-to-replace-eslint-and-prettier-1p8e)
- [Say Goodbye to Changelog Chaos: Introducing changesetgoo](https://dev.to/chandu_bobbili_06/say-goodbye-to-changelog-chaos-introducing-changesetgoo-c2j)

---

## Let's connect

- **Portfolio:** [chandubobbili.dev](https://chandubobbili.dev)
- **LinkedIn:** [chandu-bobbili](https://linkedin.com/in/chandu-bobbili-15863319b)
- **Email:** chandubobbili12@gmail.com

---

*"Great engineering merges elegant code with real-world impact."*
